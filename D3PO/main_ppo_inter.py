...
# Set up agent
    ...
    # TODO: In order for the code to run one must replace n=0 to n=10, n=20 or n=30 to change the number of training weights. 
    # Example:
    # train_weights = torch.tensor(np.array(equally_spaced_weights(dim=3, n=10)), dtype=torch.float32)
    # Above represents an experiment that provides the agent with 10 training weights
    
    eval_weights = np.array(get_eval_weights())
    train_weights = torch.tensor(np.array(equally_spaced_weights(dim=3, n=0)), dtype=torch.float32)
    ...
    trained_agent = ppo.learn(total_timesteps, eval_envs, train_weights, eval_weights) 
