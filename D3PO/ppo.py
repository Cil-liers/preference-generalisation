...
class PPO:
    ...
    def learn(self, total_timesteps, eval_envs=None, train_weights=None, eval_weights=None):
        """
        ...
        self._global_step = 0
        self.fixed_train_weights = train_weights
        current_weights = None
        
        eval_weights_is_none = eval_weights is None
        if train_weights is not None:
            index = torch.randint(0, train_weights.shape[0], (self.num_envs, ))
            current_weights = train_weights[index]
        ...
        for update in range(num_policy_updates):
            ...
            (
                ...
                current_weights,
                current_negatives
            ) = self.collect_rollouts(next_observation, is_next_observation_terminal, is_next_observation_truncated, current_weights, current_negatives, eval_weights_is_none)
            ...
            if update % self.eval_updates_freq == 0:   # every n PPO updates
                eval_archive = ParetoArchive()

                if eval_weights is not None:
                    updated_eval_archive = self.evaluate_at_weights(eval_envs, eval_weights, 5, self.device, eval_archive)
                    metrics = evaluate_agent_metrics(
                        updated_eval_archive,
    ...
    def evaluate_at_weights(self, envs, eval_weight_list, num_episodes_per_weight, device, eval_archive):

        idx = 0
        for eval_weight in eval_weight_list:
            weight = torch.tensor(eval_weight, dtype=torch.float32, device=device).unsqueeze(0).repeat(envs.num_envs, 1)
            episodes_completed = 0
            obs, _ = envs.reset()
            while episodes_completed < num_episodes_per_weight:
                action, _ = self.agent.predict(obs, weight = weight, deterministic = True, device = device)
                obs, reward, terminated, truncated, infos = envs.step(action)
                
                if "episode" in infos:
                    
                    for current_return in infos["episode"]["dr"][infos["_episode"]]:
                        eval_archive.add(tuple(eval_weight), current_return)
    ...
    def collect_rollouts(self, next_observation, is_next_observation_terminal, is_next_observation_truncated, current_weights, current_negatives, eval_weights_is_none):
        """
        ...
        for step in range(self.num_rollout_steps):
            ...
            if self.reward_size != 1 and change_weights.any():
                # print(weights)   
                if self.fixed_train_weights is not None:
                    
                    amount_of_indices = int(change_weights.sum().item())
                    index = torch.randint(0, self.fixed_train_weights.shape[0], (amount_of_indices,), device=self.device)
                    weights[change_weights] = self.fixed_train_weights[index]
